...
functions {
  ...
  // construction of the current likelihood and power prior
  real prior_likelihood_calc(int J, int[] sub_index, vector y, matrix x, vector beta, 
       matrix z, matrix Omega, vector tau, real sigmasq, real power) {
  ...
  multi_normal_lpdf( y[sub_first : sub_last] | x[sub_first : sub_last] * beta,
  z[sub_first : sub_last]*quad_form_diag(Omega, tau)*z[sub_first : sub_last]'+
...
//parameter block
parameters {
  vector[K0] beta_mutual;              // mutual fixed effects       
...
model {
  ...
  target += 
  // current likelihood
  prior_likelihood_calc(J, sub_index, y, x, append_row(beta_mutual, beta_trt), 
                        z, Omega, tau, sigmasq, 1) +  
  // power prior distribution without normalizing
  prior_likelihood_calc(J0, sub_index0, y0, x0, beta_mutual, 
                        z0, Omega, tau, sigmasq, power) -  
