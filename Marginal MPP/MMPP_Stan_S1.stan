...
model {
  ...
  for (j in 1:J)
    target += power * 
    multi_normal_lpdf(y[sub_index[2*j-1] : sub_index[2*j]] |  
    x[sub_index[2*j-1] : sub_index[2*j]] * beta, 
    z[sub_index[2*j-1] : sub_index[2*j]]*quad_form_diag(Omega, tau)*z[sub_index[2*j-1] : sub_index[2*j]]' + 
