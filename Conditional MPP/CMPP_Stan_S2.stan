...
functions {
  ...
  //construction of power prior distribution or current likelihood (power=1, data is current data)
   real prior_likelihood_calc(int J, int[] sub_index, int Nobs, vector y, matrix x, matrix z, vector beta, real power, matrix Omega, vector tau, real sigmasq) {
   ...
   for (j in 1: J) {
    ...
    //the likelihood with random effects integrated out
    prior_likelihood_sub[j] = 0.5 * Nobs * (1 - power) * log(2*pi()*sigmasq) -
    0.5 * Nobs * log(power) +
    ...
    diag_matrix(rep_vector(sigmasq/power, sub_index_upper-sub_index_lower+1)));
   }
...
model {
  ...
  target += 
  prior_likelihood_calc(J, sub_index, Nobs, y, x, z, append_row(beta_mutual, beta_trt), 1, Omega, tau, sigmasq) +  //current likelihood
            prior_likelihood_calc(J0, sub_index0, Nobs, y0, x0, z0, beta_mutual, power, Omega, tau, sigmasq) -  //power prior without normalizing
