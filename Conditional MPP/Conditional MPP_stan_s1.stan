...
data {
  ...
  matrix[N0, K0] x0;               // design matrix for the fixed effects
  matrix[N0, L] z0;                // design matrix for the random effects
  vector[N0] y0;                   // responses of the subjects
...
model {
  ...
  target += power * (multi_normal_lpdf(y0[sub_first : sub_last] |
            x0[sub_first : sub_last] * beta + 
            z0[sub_first : sub_last] * to_vector(b0[j, ]),
            diag_matrix(rep_vector(sigmasq, sub_last - sub_first+1)))) +
            multi_normal_lpdf(b0[j, ] | rep_vector(0, L), quad_form_diag(Omega, tau));
  }
}
